...
public class TableBuilderImpl implements TableBuilder {
    ...
    /*
     * @Override public final void checkTable() { boolean moves; final Check check =
     * new SimpleCheck(); final Pop pop = new PopImpl(); final Refill refill = new
     * Refill(); while (moves) { moves = false; for (int i = 0; i < this.rows; i++)
     * { for (int j = 0; j < this.columns; j++) { final Pair p = new Pair(i, j);
     * final List<Pair> result; result = check.checkMatch(p, this.grid); if
     * (!result.isEmpty()) { pop.removeCandy(result, this.grid);
     * refill.scrollDown(this.grid); moves = true; i = this.rows; j = this.columns;
     * alreadyChecked = false; } } } } alreadyChecked = true; }
     */
    @Override
    public final Table build() {
        ...
        if (this.colors.isEmpty()) {
            throw new IllegalArgumentException(
                    "You can build the table if you haven't already choose the color of the candies");
        }
        ...
        /*
         * if (!this.alreadyChecked) { throw new
         * IllegalStateException("You can build the table if you haven't checked it"); }
         */
